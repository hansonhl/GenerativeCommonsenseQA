...
def get_gpt2_position_ids(q_input_ids, question_lens, pos_id_offsets,
                          a_input_ids, include_q=True, formatting="SEP"):
    batch_size = pos_id_offsets.shape[0]
    question_lens = question_lens.unsqueeze(1)
    max_q_len = q_input_ids.shape[1]

    question_pos_ids = torch.arange(max_q_len).repeat(batch_size, 1)
    sep_pos_ids1 = question_lens.detach().clone()
    concept_pos_ids = pos_id_offsets + question_lens
    if formatting == "SEP":
        concept_pos_ids += 1
    sep_pos_ids2, _ = concept_pos_ids.max(dim=1, keepdims=True)
    sep_pos_ids2 += 1
    # sep_pos_ids2 = torch.full((batch_size, 1), max_q_len + 1 + max_pos_id_len, dtype=torch.long)

    if a_input_ids is None:
        if include_q and formatting == "SEP":
            ids = [question_pos_ids, sep_pos_ids1, concept_pos_ids, sep_pos_ids2]
        elif include_q:
            ids = [question_pos_ids, concept_pos_ids]
        elif not include_q and formatting == "SEP":
            ids = [sep_pos_ids1, concept_pos_ids, sep_pos_ids2]
        else:
            ids = [concept_pos_ids]
    else:
        max_answer_len = a_input_ids.shape[1]
        answer_pos_ids = torch.arange(max_answer_len).repeat(batch_size, 1) + sep_pos_ids2
        if formatting == "SEP": answer_pos_ids += 1
        if include_q and formatting == "SEP":
            ids = [question_pos_ids, sep_pos_ids1, concept_pos_ids, sep_pos_ids2, answer_pos_ids]
        elif include_q:
            ids = [question_pos_ids, concept_pos_ids, answer_pos_ids]
        elif not include_q  and formatting == "SEP":
            ids = [sep_pos_ids1, concept_pos_ids, sep_pos_ids2, answer_pos_ids]
        else:
            ids = [concept_pos_ids, answer_pos_ids]
