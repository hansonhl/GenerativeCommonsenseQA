...
def generate_answers(model, dataloader, tokenizer, args):
    ...
    verbose_outputs = [] if getattr(args, "verbose_generation_output", False) else None
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Generate Answers"):
            ...
            if verbose_outputs is not None:
                for j, qid in enumerate(batch["qids"]):
                    ex_dict = {}
                    ex_dict["qid"] = qid
                    iids = input_ids[j]
                    _iids = iids[iids != tokenizer.pad_token_id]
                    ex_dict["ctx"] =  tokenizer.decode(_iids)
                    ex_dict["answer"] = answer_strs[j]
                    ex_dict["gold_acs"] = batch["ac_strs"][j]
                    verbose_outputs.append(ex_dict)
